...
/// </summary>
public sealed class ConstrainedTriangulatorDOTS : Triangulator
{
    /// <summary>
    ...
    /// </summary>
    private List<EdgeConstraintDOTS> constraints;
    ...
    /// <returns></returns>
    public ConstrainedTriangulatorDOTS(List<MeshVertex> inputPoints, List<EdgeConstraintDOTS> constraints, Vector3 normal)
    ...
    internal void ApplyConstraints()
    {
        ...
        foreach (EdgeConstraintDOTS constraint in constraints)
        {
            ...
            Queue<EdgeConstraintDOTS> intersectingEdges = FindIntersectingEdges(constraint, vertexTriangles);
    ...
    internal Queue<EdgeConstraintDOTS> FindIntersectingEdges(EdgeConstraintDOTS constraint, int[] vertexTriangles)
    {
        Queue<EdgeConstraintDOTS> intersectingEdges = new Queue<EdgeConstraintDOTS>();

        // Need to find the first edge that the constraint crosses.
        EdgeConstraintDOTS startEdge;
        ...
        while (!finalTriangleFound)
        {
            ...
            {
                edgeIndex = E12;
                var edge = new EdgeConstraintDOTS(triangulation[t, V1], triangulation[t, V2], t, triangulation[t, E12], edgeIndex);
                intersectingEdges.Enqueue(edge);
            }
            else if ((triangulation[t, E23] != lastTriangle) && MathUtils.LinesIntersect(v_i, v_j, v2, v3))
            {
                edgeIndex = E23;
                var edge = new EdgeConstraintDOTS(triangulation[t, V2], triangulation[t, V3], t, triangulation[t, E23], edgeIndex);
                intersectingEdges.Enqueue(edge);
            }
            else if ((triangulation[t, E31] != lastTriangle) && MathUtils.LinesIntersect(v_i, v_j, v3, v1))
            {
                edgeIndex = E31;
                var edge = new EdgeConstraintDOTS(triangulation[t, V3], triangulation[t, V1], t, triangulation[t, E31], edgeIndex);
                intersectingEdges.Enqueue(edge);
            }
            else
            {
    ...
    internal bool FindStartingEdge(int[] vertexTriangles, EdgeConstraintDOTS constraint, out EdgeConstraintDOTS startingEdge)
    {
        // Initialize out parameter to default value
        startingEdge = new EdgeConstraintDOTS(-1, -1);
        ...
        if (intersectionFound)
        {
            ...
            startingEdge = new EdgeConstraintDOTS(v_k, v_l, tSearch, triangle2, intersectingEdgeIndex);
    ...
    internal void RemoveIntersectingEdges(EdgeConstraintDOTS constraint, Queue<EdgeConstraintDOTS> intersectingEdges)
    {
        // Remove intersecting edges. Keep track of the new edges that we create
        List<EdgeConstraintDOTS> newEdges = new List<EdgeConstraintDOTS>();
        EdgeConstraintDOTS edge, newEdge;
        ...
        while (intersectingEdges.Count > 0 && counter <= intersectingEdges.Count)
        {
            ...
            if (FindQuadFromSharedEdge(edge.t1, edge.t1Edge, out quad))
            {
                        ...
                        points[quad.q2].coords))
                {
                    ...
                    newEdge = new EdgeConstraintDOTS(quad.q3, quad.q4, quad.t1, quad.t2, E31);
    ...
    internal void RestoreConstrainedDelauneyTriangulation(EdgeConstraintDOTS constraint, List<EdgeConstraintDOTS> newEdges)
    {
        ...
        while (swapOccurred)
        {
            ...
            {
                EdgeConstraintDOTS edge = newEdges[i];
                ...
                if (FindQuadFromSharedEdge(edge.t1, edge.t1Edge, out quad))
                {
                    if (SwapTest(points[quad.q1].coords, points[quad.q2].coords, points[quad.q3].coords, points[quad.q4].coords))
                    {
                        ...
                        int v_m = quad.q3;
                        int v_n = quad.q4;
                        newEdges[i] = new EdgeConstraintDOTS(v_m, v_n, quad.t1, quad.t2, E31);
    ...
    internal void DiscardTrianglesViolatingConstraints()
    {
        ...
        {
            EdgeConstraintDOTS constraint = this.constraints[i];
    ...
    internal bool TriangleContainsConstraint(int t, EdgeConstraintDOTS constraint)
    {
    ...
    internal bool EdgeConstraintIntersectsTriangle(int t, EdgeConstraintDOTS constraint, out int intersectingEdgeIndex)
    {
        Vector2 v_i = points[constraint.v1].coords;
        Vector2 v_j = points[constraint.v2].coords;
        Vector2 v1 = points[triangulation[t, V1]].coords;
    ...
    internal void SwapQuadDiagonal(Quad quad, IEnumerable<EdgeConstraintDOTS> edges1, IEnumerable<EdgeConstraintDOTS> edges2, IEnumerable<EdgeConstraintDOTS> edges3)
    {
        // BEFORE
        //               q3        
        //      *---------*---------*
    ...
    internal void UpdateEdgesAfterSwap(IEnumerable<EdgeConstraintDOTS> edges, int t1, int t2, int t1L, int t1R, int t2L, int t2R)
    {
        if (edges == null)
        {
            return;
        ...
        foreach (EdgeConstraintDOTS edge in edges)
        {
